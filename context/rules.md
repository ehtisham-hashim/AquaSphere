...
This document tells anyone (or any AI) writing code for this project **how** to write it. `AQUA_Sphere_OS_Master_Requirements.md` says what to build, `architecture.md` says how the system is shaped, and this file is the rulebook that keeps every file that gets written consistent with both — no matter who or what writes it.
...
- Shared logic (like mineral-set math, preform conversion, balance calculation) lives in `backend/src/services/` or `backend/src/utils/`, imported where needed. Do not copy-paste.
...
- Nothing business-specific goes in `middleware/` — that folder is only for truly generic, reusable code (auth, validation, error handling, company-context routing, daily-close guard).
...
- **Distinguish real errors from soft-block warnings.** A credit-limit warning, a stock-going-negative warning, or a bottle-return-overage warning is *not* an error — it's a normal `200` response with a `warning` flag (see `architecture.md` §9). Reserve actual HTTP error codes (400/403/404/500) for genuine problems: bad input, missing records, unauthorized access, server failure.
...
- **Every soft-block check (Section 8 of Master Requirements) must be enforced server-side**, even though the frontend should also show the warning early for a good user experience. The frontend check is a convenience; the backend check is the real gate.
...
- **Vendor must exist before purchase.** The purchase form CANNOT be submitted without a valid, existing vendor attached. There is no "quick add" shortcut to create a vendor halfway through filling out a purchase.
- **Credit limit = 0 means "unlimited"** — NOT "block everything." The soft-block check must be skipped when `creditLimit === 0`.
...
- **Role checks on every protected route** via `role.middleware.ts` — don't rely on the frontend hiding a button as the only protection. If Marketing Manager shouldn't see profit margins, the API must refuse to return that data to a Marketing Manager token.
...
- **Daily-closed records are backend-enforced as locked** — ALL roles (Marketing Manager, Production Manager, Accountant, Admin) must be rejected by the API from writing transactions dated on/before the last close. Only Owner can override.
...
- **No local/session storage for anything business-critical** (balances, tokens, company context) — httpOnly cookies and in-memory React state only.

## 7. Division Isolation Rules
...
- **No local/session storage for anything business-critical** (balances, tokens meant to be secure, company context) — httpOnly cookies and in-memory state only.
...
- **Never invent business rules.** If a number, formula, or workflow isn't in `AQUA_Sphere_OS_Master_Requirements.md`, don't guess — flag it as an open question.
