  ...
  if (!isWadaana) {
    ...
    return (
      <div className="space-y-4">
        ...
        <div className="card-surface p-3.5 sm:p-4 flex flex-col md:flex-row md:items-center justify-between gap-3">
            ...
            <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs sm:text-sm text-slate-600">
              {fgList.map((item, idx) => (
                <span key={item.id || idx} className="flex items-center gap-1">
                  {item.name}: <strong className="text-slate-900 font-mono font-bold text-sm sm:text-base">{Math.round(Number(item.cachedQty || 0)).toLocaleString()}</strong>
                </span>
              ))}
            </div>
          </div>
          ...
          <div className="flex items-center gap-4">
            <div>
              <span className="text-[11px] sm:text-xs uppercase font-bold text-slate-400 tracking-wider block">Total Finished Stock</span>
              <span className="text-2xl font-mono font-bold text-brand">{Math.round(totalUnits).toLocaleString()} <span className="text-xs font-normal text-slate-400 font-sans">Units</span></span>
            </div>
          </div>
        </div>
        ...
        return (
          <div key={idx} className="card-surface p-4 space-y-2.5">
            ...
            <div className="grid grid-cols-2 gap-2 pt-2.5 border-t border-slate-100 text-xs">
              <div>
                <span className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider block mb-0.5">Factory</span>
                <span className="text-sm sm:text-base font-mono font-bold text-slate-800">{factoryVal.toLocaleString()}</span>
              </div>
              <div>
                <span className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider block mb-0.5">Warehouse</span>
                <span className="text-sm sm:text-base font-mono font-bold text-brand">{warehouseVal.toLocaleString()}</span>
              </div>
            </div>
          </div>
        );
