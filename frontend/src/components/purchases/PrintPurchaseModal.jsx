      ...
      <div className="bg-white rounded-2xl w-full max-w-xl shadow-2xl overflow-hidden border border-slate-100 p-8 space-y-6">
        <div className="flex justify-between items-start border-b border-slate-200 pb-4">
          ...
          <div className="text-right">
            <div className="text-sm font-bold text-slate-800">{purchase.invoiceNo || purchase.id}</div>
        ...
        <div className="grid grid-cols-2 gap-4 text-xs bg-slate-50 p-4 rounded-xl">
          ...
          <div>
            ...
            <span className="font-bold text-indigo-700 text-sm">{purchase.deliveredTo || 'FACTORY'}</span>
          </div>
        </div>
        ...
        <div className="flex justify-between items-center pt-2 border-t border-slate-200">
          ...
          <span className="text-2xl font-black text-indigo-600">Rs {Number(purchase.grandTotal).toLocaleString('en-PK')}</span>
        </div>

        <div className="flex justify-end gap-3 pt-4 border-t border-slate-100">
          <button onClick={onClose} className="px-4 py-2 text-slate-600 font-semibold text-xs hover:bg-slate-100 rounded-xl">
            Close
          </button>
