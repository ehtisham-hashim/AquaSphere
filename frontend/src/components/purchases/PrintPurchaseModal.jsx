    ...
    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-2xl w-full max-w-xl shadow-2xl overflow-hidden border border-slate-100 p-8 space-y-6">
        <div className="flex justify-between items-start border-b border-slate-200 pb-4">
          <div>
            <h2 className="text-xl font-bold text-brand">PURCHASE INVOICE</h2>
        ...
        <div className="grid grid-cols-2 gap-4 text-xs bg-slate-50 p-4 rounded-xl border border-slate-200">
          ...
          <div>
            ...
            <span className="font-bold text-brand text-sm">{purchase.deliveredTo || 'FACTORY'}</span>
          </div>
        </div>
        ...
        <div className="flex justify-between items-center pt-2 border-t border-slate-200">
          ...
          <span className="text-xl font-bold font-mono text-brand">Rs {Number(purchase.grandTotal).toLocaleString('en-PK')}</span>
        </div>
